...
// Implements the current operator as sparse matrix to compute off diagonals elements
// Other local magnetisation operators are in the code
  ...
  for(MKL_INT i = 0; i < basis_size; ++i){
    for(MKL_INT j = 0; j < (i + 1); ++j){
      ...
      if( ( (std::abs(eigvals[i] + eigvals[j])) / l ) <= window ){
        ...
        std::cout << eigvals[i] - eigvals[j] << " " << std::abs(val4) << std::endl;
      }
    }
  }
  toc = seconds();
